...
CONFIG = {
    ...
    # האתר למעקב
    "blog_url": "https://newsmusic.blogspot.com/",
    ...
    # קבצים מקומיים
    "download_dir": os.path.join(DATA_DIR, "downloads_newsmusic"),
    "tts_dir": os.path.join(DATA_DIR, "tts_newsmusic"),
    "compressed_dir": os.path.join(DATA_DIR, "compressed_newsmusic"),
    "state_file": os.path.join(DATA_DIR, "state_newsmusic.json"),

    # רשת
    "timeout": 60,
    "user_agent": "Mozilla/5.0 (Macintosh; Intel Mac OS X) NewsMusicToYemot/4.0",
...
logger = logging.getLogger("newsmusic-ext3")
...
def build_tts_text(title: str) -> str:
    ...
    return f"{CONFIG['tts_prefix']} {title}"
...
def compress_audio_if_needed(input_path: str) -> str:
    ...
    logger.info(f"נוצר קובץ דחוס: {output_path} ({compressed_size_mb:.2f}MB)")
...
def upload_file_to_yemot(local_path: str):
    ...
    with open(local_path, "rb") as f:
        files = {
            "file": (os.path.basename(local_path), f, mime_type)
        }
...
def process_once():
    ...
    for entry in entries:
        ...
        logger.info(f"קישור פוסט: {post_url}")
...
def main():
    ...
    logger.info("הסקריפט התחיל")
    logger.info(f"שלוחה יעד: {CONFIG['target_extension']}")
    logger.info(f"בדיקה כל {CONFIG['check_interval_seconds']} שניות")
    logger.info("בריצה ראשונה: לא מעלה אחורה, רק מאותחל על הקיים")
