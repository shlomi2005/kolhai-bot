...
logger = logging.getLogger("musiclik-ext300")
...
def get_api_key() -> str:
    api_key = os.environ.get("YEMOT_API_KEY_MUSICLIK", "")
    api_key = api_key.strip().strip("'\"׳״` ")

    if not api_key:
        raise RuntimeError("לא נמצא YEMOT_API_KEY_MUSICLIK במשתני הסביבה")

    try:
        api_key.encode("latin-1")
    except UnicodeEncodeError:
        raise RuntimeError(f"YEMOT_API_KEY_MUSICLIK מכיל תווים לא תקינים: {repr(api_key)}")
...
def main():
    ...
    logger.info("הסקריפט התחיל — מיוזיקליק שלוחה 300")
